...
Installing through Homebrew installs them, so `man grat` works without a further
step. There are two:

| Page | Shows |
| --- | --- |
| `grat(1)` | Every command, and how grat starts, watches and stops a service. Reachable as `man grat`, `man 1 grat` or `man grat.1`. |
| `grat.config(7)` | Every option the configuration file takes. Reachable as `man grat.config` or `man 7 grat.config`. |

Both are generated by grat rather than written alongside it, so they can only
describe the binary they came from. The command page is built from the same
reference `grat help` prints, and the configuration page takes its defaults,
port ranges and inherited variables from the code that enforces them. `grat
manual` writes the first to standard output and `grat manual grat.config` the
second, which is what the Homebrew formula and the release workflow install.

A release also carries both beside the binaries, for whoever installs a binary
by hand:
...
`go install` places a binary and nothing else, so that route carries no man
pages.
