...
The page is generated by grat itself rather than written alongside it, so it can
only describe the binary it came from. `grat manual` writes it as roff to
standard output, which is what the Homebrew formula and the release workflow
both install. A release also carries the page as `grat.1` beside the binaries,
for whoever installs a binary by hand:
...
`grat help` prints the same command reference the page contains.
...
`grat init` asks for the project name and proposes the services it recognises,
as described in [What grat recognises](#what-grat-recognises). Review each
suggestion before accepting it. A project grat cannot resolve, and any service
it does not propose, can be given explicitly:
