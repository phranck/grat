<p align="center">
  <a href="https://github.com/phranck/grat/actions/workflows/ci.yml"><img alt="CI" src="https://img.shields.io/github/actions/workflow/status/phranck/grat/ci.yml?branch=main&label=CI&color=e6194b"></a>
  <a href="https://github.com/phranck/grat/releases"><img alt="Release" src="https://img.shields.io/github/v/release/phranck/grat?display_name=tag&label=release&color=f58231"></a>
  <a href="go.mod"><img alt="Go" src="https://img.shields.io/github/go-mod/go-version/phranck/grat?label=go&color=d4a017"></a>
  <a href="https://github.com/phranck/grat/releases"><img alt="Downloads" src="https://img.shields.io/github/downloads/phranck/grat/total?label=downloads&color=3cb44b"></a>
  <a href="LICENSE"><img alt="License" src="https://img.shields.io/github/license/phranck/grat?color=008080"></a>
...
`grat discover` reads the project and proposes the services it recognises, so most projects need nothing written by hand. Angular, Astro, Next.js, Nuxt, React Router, SvelteKit and Vite are recognised on the frontend, Django, Laravel, Symfony, Rails, Phoenix, Spring Boot, FastAPI, Flask and ASP.NET Core on the backend, along with Go modules, Rust crates using Axum or Actix, Deno and Bun projects, Hugo and Jekyll sites, and Swift packages using Vapor. A Docusaurus site or a Storybook is reported in addition to the application it sits beside, on a port of its own.

Express, Fastify, NestJS and Go are different: none of them reads `PORT` on its own and none has a flag for it, so the port lives in the line of your own code that starts the server. grat looks there, and where it does not find it, it says so instead of proposing a command that would never become ready. Eleventy is refused for its own reason: its development server binds every interface with no flag to pin it to loopback, and it moves to another port when the assigned one is taken. [What grat recognises](Documentation.md#how-grat-decides-what-a-project-runs) explains both.
...
- **[Documentation.md](Documentation.md)** for the configuration reference, the roles and their port ranges, and what readiness and shutdown guarantee.
