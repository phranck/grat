...
Astro, React Router, SvelteKit, Django, Rails, Symfony, Flask, ASP.NET Core and
Hugo all do, and for those grat writes the flag into the command and the matter
is settled. A Go module yields one service per program below cmd, and a Swift
package depending on Vapor yields its executable target.

A static site is a service like any other, and the three generators grat knows
each answer the port question differently. Hugo reads nothing beyond its flags,
so its configuration file is the whole of what grat needs. Jekyll takes the same
two flags whilst its command runs through bundler, so a site is proposed only
where the Gemfile declares the jekyll gem, and --detach is left out because a
server that backgrounds itself leaves nothing to watch. Eleventy is recognised
in order to be refused: its development server binds every interface and its
argument parser rejects --host, so nothing keeps the site off the network, and
it moves to another port when the assigned one is taken, with no equivalent of
Vite's --strictPort to stop it. grat reports an Eleventy project with that
...
port there wins over everything, so that is the case grat refuses.
...
long-lived daemon by default, and the application would then hang below that
daemon rather than below the command grat started, so the port is held and
readiness never arrives. On Maven the plugin forks its own child and needs no
flag. Phoenix has no port flag at all and takes the port only where
config/runtime.exs reads it, which the project generator writes and a project can
remove; where that line is absent grat reports the project rather than proposing
...
grat runs a grat.config only where you decided what it says. The search for one
walks upward from the current directory and stops where a directory belongs to
another account, and a file that belongs to somebody else, or that its group or
everybody can write, is refused by name rather than read. A configuration names
commands that run as whoever typed the grat command, so who chose the file is the
same question as what it runs.

Managed state and logs sit under .grat with restrictive permissions. That
directory sits inside the project, and a repository decides what it brings with
it, so grat refuses a symbolic link in place of the log or of either state
directory rather than writing through it. A startup failure stops what that operation
started, removes its state, and reports the closing lines of the service's log
rather than only a timeout. An interrupted start cleans up the same way.

A name grat reads out of a project's own files, such as an executable target or
a directory below cmd, reaches a command only where it is letters, digits,
underscore, hyphen or full stop. Anything else is reported with the file and the
character rather than written into a command line.
